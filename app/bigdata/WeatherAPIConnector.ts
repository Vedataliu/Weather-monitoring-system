...
export class WeatherAPIConnector {
  ...
  private readonly API_KEY = process.env.NEXT_PUBLIC_OPENWEATHER_API_KEY || process.env.OPENWEATHER_API_KEY || ''
  ...
  public async fetchCityWeatherByCoords(lat: number, lng: number): Promise<CityData | null> {
    try {
      if (!this.API_KEY) {
      ...
      console.error(`Error fetching weather data for coords ${lat}, ${lng}:`, error)
      return null
    }
  }

  public async fetchCityWeatherByName(cityName: string, countryCode?: string): Promise<CityData | null> {
    try {
      if (!this.API_KEY) {
        console.warn('⚠️ OpenWeatherMap API key not found. Using fallback data.')
        return null
      ...
      const url = `${this.API_BASE_URL}/weather?q=${encodeURIComponent(query)}&appid=${this.API_KEY}&units=metric`
  ...
  public async fetchMultipleCitiesWeather(limit?: number): Promise<CityData[]> {
    ...
    for (let i = 0; i < citiesToFetch.length; i += batchSize) {
      ...
      const batchPromises = batch.map(async (city) => {
        try {
          ...
          let cityData = await this.fetchCityWeatherByCoords(city.lat, city.lng)
          
          // Fallback to city name if coordinates fail
          if (!cityData) {
            cityData = await this.fetchCityWeatherByName(city.name, city.country)
          }
    ...
    console.log(`✅ Successfully fetched OpenWeatherMap data for ${results.length} cities`)
    return results
  }
  ...
  private transformWeatherData(weatherData: OpenWeatherMapResponse): CityData {
    ...
    const weatherDescription = weatherData.weather[0]?.description || 'clear sky'
    
    // Convert weather condition to health level for compatibility
    const healthLevel = this.getHealthLevelFromWeather(weatherMain, weatherData.main.temp)
    ...
    const precipitation = this.estimatePrecipitation(weatherData.main.humidity, weatherData.clouds?.all || 0)
    ...
    return {
      ...
      aqi: this.calculateWeatherIndex(weatherData.main.temp, weatherData.main.humidity, weatherData.wind?.speed || 0),
  ...
  private getHealthLevelFromWeather(weatherMain: string, temperature: number): string {
  ...
  private calculateWeatherIndex(temp: number, humidity: number, windSpeed: number): number {
  ...
  public async getGlobalInsights(citiesData?: CityData[]): Promise<{
    ...
    try {
      ...
      const totalTemp = data.reduce((sum, city) => sum + (city.temperature || 20), 0)
      ...
      const citiesWithAlerts = data.filter(city => 
